...
| Команда | Описание |
|---------|----------|
| `/lmem status` | Показать статус готовности плагина и основных компонентов |
| `/lmem health` | Показать оценку состояния, затронутые области и фиксированные рекомендации |
| `/lmem diagnostics` | Показать текущий снимок Provider, поиска, задач, индекса и записи |
| `/lmem search <query> [k]` | Поиск памяти, `k=5` по умолчанию |
| `/lmem trace <query> [k]` | Трассировать этапы и оценки поиска текущей сессии без вывода содержимого памяти в чат |
| `/lmem forget <doc_id>` | Удалить конкретную запись памяти |
| `/lmem rebuild-index` | Перестроить векторные/BM25 индексы |
| `/lmem rebuild-graph` | Перестроить индексы графовой памяти |
| `/lmem webui` | Показать информацию о доступе к WebUI |
| `/lmem summarize` | Немедленно запустить суммаризацию текущей сессии |
| `/lmem reset` | Сбросить контекст долговременной памяти текущей сессии |
| `/lmem cleanup [preview\|exec]` | Очистить фрагменты инъекций памяти из исторических сообщений |
| `/lmem update [check\|download\|apply]` | Проверить, скачать или установить обновление runtime с проверкой SHA-256; `apply` перезагружает и откатывает версию при поддержке хоста |
| `/lmem help` | Показать справку по командам |
