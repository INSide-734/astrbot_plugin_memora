...
Храня канонические записи в SQLite как единственный авторитетный источник, Memora сохраняет возможность перестроить полнотекстовые, векторные, графовые и проекционные представления — позволяя вашему боту по-настоящему «помнить» каждый разговор.
...
- **Веб-панель управления** — React + shadcn/ui админ-панель с 17 страницами
...
| **Injection Strategy** | Настройка маршрутизации инъекций и просмотр обезличенных решений |
| **Intelligence** | Управление когнитивными сигналами |
| **Jargon** | Поиск и разбор жаргона |
| **Affection** | Просмотр состояния симпатии |
| **Social** | Просмотр социальных отношений |
| **Config** | Редактирование конфигурации плагина |
| **Gate** | Настройка правил ворот записи памяти |
...
Memora использует фреймворк pytest с поведенческими, контрактными и интеграционными наборами тестов, охватывающими основную функциональность.
...
│   ├── platform/              # Конфигурация, композиция, транспорт, провайдеры, безопасность
│   ├── shared/                # Общие порты, DTO, SQL-константы, чистые утилиты
│   ├── features/              # Функциональные модули (memory, recall, retrieval, injection, ...)
│   ├── prompts/               # Шаблоны промптов
│   ├── i18n/                  # Интернационализация (zh / en / ru)
│   ├── event_handler.py       # Точки входа обработки событий
│   └── event_cognitive.py     # Захват когнитивных событий
│
├── pages/dashboard/           # Веб-панель администратора
│   └── src/pages/             # 17 функциональных страниц
│
├── tests/                     # Поведенческие, контрактные и интеграционные наборы pytest
├── scripts/                   # Quality gate, бенчмарки, миграции и упаковка
├── docs/                      # Документация репозитория (AGENTS.md, DEV_SETUP.md)
├── website/                   # Сайт документации VitePress
├── assets/                    # Скриншоты Dashboard для документации
├── static/                    # Встроенные статические данные (stopwords)
├── skills/                    # Встроенные навыки агента
└── .github/                   # CI, формы Issue и политики репозитория
```
