...
SQLite 中的 canonical memory（`documents` 行 + 整数 ID）是唯一权威记录，也是唯一权威事实正文。正文更新采用两阶段替换：新内容先落为不可召回的暂存记录，再在单个事务内切换可见性（赢家恢复原状态、输家隐藏），随后单独删除旧记录，因此任一时刻最多只有一条可召回记录；删除未完成时由写账本记为待修复，失败只记录不回滚。
