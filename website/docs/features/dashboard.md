...
图谱页面打开后自动显示最近七天的管理员总览，可同时浏览多个会话和群的有效图数据。直接输入实体、主题或节点名称即可搜索并查看一跳关联；记忆 ID 仅用于可选聚焦，不是浏览前提。

“最近概览”会清空搜索和记忆 ID，并恢复最近七天；“应用”提交时间范围，“重置”显示全部时间。没有命中时可以调整筛选，不需要随意填入记忆 ID。

总览只展示仍能通过当前 canonical 来源、隐私、修订和用户证据校验的节点与边；不会合并不同边界下的同名节点。旧图缺失来源、来源已变更或尚未成功建图时仍会为空，恢复浏览方式不会修复或绕过这些数据问题。管理员视图不改变普通聊天的跨会话和隐私隔离。
