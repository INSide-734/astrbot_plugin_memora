...
class StopwordsManager:
    ...
    async def load_stopwords(
        ...
        返回:
            停用词集合。
        """
        logger.info(f"开始加载停用词表：来源={source}")
        ...
        if custom_words:
            ...
            logger.info(f"已添加自定义停用词：{len(custom_words)} 个")

        logger.info(f"停用词表加载完成，共 {len(self.stopwords)} 个词")
        return self.stopwords

    @staticmethod
    async def _load_from_file(filepath: Path) -> set[str]:
        """
        从文件加载停用词
        ...
        try:
            ...
            logger.debug(f"已从文件加载停用词：{filepath}，共 {len(stopwords)} 个")
            return stopwords
    ...
    @staticmethod
    def _get_builtin_stopwords() -> set[str]:
        """
        获取内置的基础停用词表（作为后备方案）
    ...
    def add_custom_stopwords(self, words: list):
        ...
        if words:
            ...
            logger.info(f"已添加 {len(words)} 个自定义停用词")
    ...
    def remove_stopwords(self, words: list):
        ...
        if words:
            ...
            logger.info(f"已移除 {len(words)} 个停用词")
    ...
    async def _write_fallback_stopwords(self, filepath: Path) -> None:
        ...
        try:
            ...
            async with aiofiles.open(filepath, "w", encoding="utf-8") as f:
                await f.write("# Generated fallback stopwords for Memora\n")
                await f.write("# 为记忆插件自动生成的后备停用词\n")
                for word in words:
                    await f.write(f"{word}\n")
