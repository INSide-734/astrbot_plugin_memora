...
- **内部向下依赖：** `core.shared.constants`、`core.features.injection.domain.models`、`core.models.default_stopwords`、`core.features.recall.processors.text_processor`（仅包级转导）。
- **外部依赖：** AstrBot API、`pytz`、`PyYAML`；`cachetools` 与 `APScheduler` 有显式降级路径。
- **被依赖：** 召回/注入路径使用格式化、预算和适配器；可观测性/处理/检索路径使用分词；插件入口使用版本；多类管理器使用缓存、停用词和调度器。
