...
- **命令端点**: `core/command_endpoints.py` -- 混入 `MemoraPlugin`，注册 `/memora` 命令组
...
**Q: 插件初始化失败怎么办？**
A: 检查 AstrBot 是否正确配置了 LLM Provider 和 Embedding Provider。插件会自动后台重试最多 60 次。使用 `/memora status` 查看状态。
