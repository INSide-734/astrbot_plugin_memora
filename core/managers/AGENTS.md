...
`core/managers/` 是业务生命周期与编排层。它把 SQLite 文档表、BM25、FAISS、记忆原子和图记忆组合成统一的 `MemoryEngine`，并提供会话、画像、知识、笔记、备份、导入导出、衰减与写故障恢复等领域服务。

本层负责“何时、按什么顺序、失败后如何补偿”；底层表 CRUD 属于 [`core/storage/AGENTS.md`](../storage/AGENTS.md)，候选召回和排序属于 [`core/retrieval/AGENTS.md`](../retrieval/AGENTS.md)，定时触发属于 [`core/schedulers/AGENTS.md`](../schedulers/AGENTS.md)。
...
| 备份 | `backup_manager.py` | 只备份白名单模式；恢复先暂存为 `*.restore`，启动前应用 |
...
4. `BackupManager.validate_backup_name()`、备案目录集合和 `relative_to(backups_root)` 共同阻止路径穿越；恢复文件还必须匹配 `_BACKUP_PATTERNS`。不可绕过这些 API 直接拼接路径。
...
- `BackupManager` 备份复制单文件失败会记录错误并继续；`files_copied` 必须用于判断备份完整度。
