...
class DecayOperationsMixin:
    ...
    async def update_access_time(
        ...
        """更新最后访问时间 + 回想增强 importance boost

        注意：高频调用场景（如 search_memories 后对每条结果更新）
        请使用 update_access_times_batch 批量版本，避免 SQLite 写锁竞争。
        """
    ...
    async def update_access_times_batch(
        ...
        """批量更新最后访问时间 + 回想增强 importance boost。

        将 N 次 SELECT + N 次 UPDATE + N 次 COMMIT 压缩为：
        1 次 SELECT WHERE id IN (...) + N 次 UPDATE + 1 次 COMMIT，
        消除 SQLite 单写者串行化造成的 80s 瓶颈。
        """
        if not memory_ids:
            return 0
        if self._db is None:
