...
图谱 `GET /graph/overview` 与无 `memory_id` 的 `graph/search` / `graph/query` 是宿主认证/授权管理视图，可跨多个会话与群浏览；记忆 ID 只用于聚焦。总览必须通过专用 `get_admin_canvas_snapshot()` 在同一只读事务内逐来源核对当前 canonical 的 scope/privacy/revision、可召回状态与 fact 用户证据，节点、边端点及边时间都必须归属同一有效来源。不得把客户端 boundary 当授权，也不得让聊天召回复用管理员读入口。管理员查询仅筛选有效画布的一跳邻域，不调用 Provider 或有强化写入副作用的 `search_memories()`。
...
- `/memories/detail` 只投影管理员 allowlist（正文/摘要/类型/状态/重要性/时间/key_facts/topics/update_history、`graph_context`、`source_replayability`）：不返回原始 `metadata`、`session_id`、`persona_id` 或任何 source mapping/scope/privacy/revision。`source_replayability` 由 `SourceReplayabilityAssessor` 只读对账 canonical 来源引用与 ConversationStore 当前消息事实，只允许 `replayable/partial/unavailable/unknown` 聚合状态、计数与固定原因码；Store 缺失、读取异常、epoch 无法对齐或引用格式不明一律 `unknown`，不得推断消息已被删除，也不得把状态复用于模型召回或聊天注入。
- `/memories` 的 canonical 来源门：`privacy_level` 缺失或为 `null` 的历史行按 `shared` 回退（与 `canonical_source_validation` 同一口径，请求端 `privacy_level` 筛选沿用该回退值），键存在但越界或非文本仍按非法排除；行还必须满足非 `summary_source_orphan`、`source_provenance_complete` 与文本 `scope_key`。
- 上述列表门比召回门更严（召回额外要求用户证据与状态，但不要求 `source_provenance_complete` 与文本 `scope_key`），因此仍可召回的 active 行可能不出现在管理员列表——这是有意差异，不通过放宽列表门消除。列表响应顶层以整数 `dropped_source_count`（与 `total` 同级）报告**在同一组请求筛选下**被该门剔除的行数：筛选含 keyword、session_id、status、include_mark_write 与请求 scope/privacy，两者共用同一段参数化 SQL，注释中不含行数以外信息，也不回显 scope/privacy/revision 取值或正文；无可剔除行时为 0。计数只解释可见性差异，不参与分页，也不改变 `total` 与 `items` 的口径。
- `/recall/test` 响应前按整数 `doc_id` 走一次批量 canonical 回读（`faiss_db.document_storage`，并提供 `db_connection` 以保留 SQLite 原始 revision）：行不存在、不可召回、mark_write、正文不一致，或候选携带的 `revision` 快照（`revision_token`，否则同一 `metadata.updated_at`）与 canonical 行当前值不一致的候选按 `dropped_stale_count` 剔除，正文与摘要只投影 canonical 行自身；回读端口缺失或读取异常返回稳定错误 `召回结果校验失败`，不得把故障伪装成空结果，也不逐条调用单行读取兜底。
...
- 动态记忆传输不提供 `system_prompt`，API catalog 的 deliveries 也不得出现该值；详见 [注入模块 AGENTS.md](../../../features/injection/AGENTS.md)。
- SSE 每客户端队列上限 256；满队列客户端被移除，30 秒心跳，断开时注销。

## 依赖方向

`platform/transport/page_api/page_api.py` → 本目录 mixin → feature services/config/injection。Dashboard 通过已注册路径消费接口；领域层不得反向导入 Page API。
...
- `core/platform/transport/page_api/page_api.py`
