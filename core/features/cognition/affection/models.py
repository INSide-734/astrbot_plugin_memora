...
INTERACTION_RULES: dict[InteractionType, _InteractionRule] = {
    ...
    InteractionType.ENCOURAGE: _InteractionRule(
        4, True, 0.25, positive_mood_boost=True
    ),
    ...
    # --- Negative ---
    InteractionType.INSULT: _InteractionRule(
        -8, True, -0.5, negative_mood_trigger=True
    ),
    InteractionType.HARASSMENT: _InteractionRule(
        -6, True, -0.4, negative_mood_trigger=True
    ),
    InteractionType.ABUSE: _InteractionRule(
        -10, True, -0.6, negative_mood_trigger=True
    ),
    InteractionType.THREAT: _InteractionRule(
        -12, True, -0.7, negative_mood_trigger=True
    ),
}
...
KEYWORD_INTERACTION_MAP: list[tuple[list[str], InteractionType]] = (
    _KEYWORD_INTERACTION_MAP
)
...
def classify_by_keywords(message: str) -> InteractionType | None:
    ...
    for keyword, itype in sorted(
        _KEYWORD_FLAT.items(),
        key=lambda kv: (-len(kv[0]), _PRIORITY_ORDER.get(kv[0], 999)),
    ):
        if keyword in msg:
            return itype
    return None
