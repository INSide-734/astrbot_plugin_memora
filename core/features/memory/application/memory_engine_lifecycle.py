...
class MemoryEngineLifecycleMixin:
    ...
    async def initialize(self):
        ...
        if bool(self.config.get("user_profile.enabled", True)):
            from ...profiles.application import ProfileManager
            from ...profiles.infrastructure.profile_store import ProfileStore
        ...
        from ...learning.application import FeedbackSignalManager
        from ...learning.application.auto_learning import AutoLearningManager
        from ...learning.domain.models import (
            FeedbackAdapterKind,
            FeedbackSignalPolicy,
        )
        from ...learning.infrastructure.auto_learning_state import (
            AutoLearningStatePersistenceError,
        )
        from ...learning.infrastructure.feedback_signal_store import (
            FeedbackSignalStore,
        )
        ...
        if bool(self.config.get("knowledge_base.enabled", True)):
            from ...knowledge.application.knowledge_manager import (
                KnowledgeManager,
            )
            from ...knowledge.infrastructure.knowledge_store import (
                KnowledgeStore,
            )
        ...
        if bool(self.config.get("notes.enabled", True)):
            from ...notes.application.note_manager import NoteManager
            from ...notes.infrastructure.note_store import NoteStore
        ...
        if bool(self.config.get("reconsolidation.enabled", False)):
            from ...reconsolidation.application.reconsolidation import (
                ReconsolidationManager,
            )
            from ...reconsolidation.infrastructure.reconsolidation_store import (
