...
`core/features/knowledge/infrastructure/KnowledgeExtractor` 只负责把有限 canonical evidence 转成 `KnowledgeEntry`，不写入 Store。
...
- `build_memory_from_structured_data()` 用于已有结构化数据；`classify_atoms_from_metadata()` 尊重 `atom_enabled`。
