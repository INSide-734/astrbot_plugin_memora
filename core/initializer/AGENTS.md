...
本目录只负责启动期基础组件装配：选择并等待 Embedding/LLM Provider、探测并延迟加载 FAISS、初始化主/图向量库与记忆组件、修复索引和会话计数，以及启动注入决策持久化。配置的默认值合并、Schema 校验、修订冲突与持久化事务属于 `core/base/config_manager.py`，业务检索、API 和命令不应下沉到这里。
...
| `ComponentFactory` | `build_all(...) -> dict` | 返回数据库、引擎、处理器、备份/会话/索引/衰减及注入决策组件字典 |
...
3. 构造并初始化 `MemoryEngine`；其配置由 `ConfigManager.get()` 逐项投影，覆盖召回、图扩展、重排、成本控制、索引重建和缓存等，而不是在工厂内再次合并配置。
4. 初始化 `conversations.db` 与 `ConversationManager`，随后修复 `message_count`。
5. 构造 `MemoryProcessor`、`IndexValidator`，执行一致性检查，并异步加载停用词。
6. 当衰减或自动清理启用时启动 `DecayScheduler`。
7. 在 `memora.db` 上初始化 `InjectionDecisionStore` 和有界异步 `InjectionDecisionRecorder`，应用保留天数与最大行数并安排清理。

若第 7 步失败，工厂按调度器、会话存储、引擎、图 DB、主 DB 的顺序尽力回滚；各关闭失败只记录日志，原异常继续传播。不要把这一局部回滚误写成覆盖前面所有装配阶段的通用事务。
...
重点保护：Provider 迟到后回调、重复重试抑制/取消、FAISS 探测失败、维度隔离、组件字典完整性、注入存储启动/失败回滚、配置 revision 冲突。仅改本模块时先跑第一个命令；涉及配置装配契约时再跑第二个。
