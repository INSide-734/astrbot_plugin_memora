...
- **调用方**: `DatabaseSetup.auto_rebuild_index_if_needed()`, `ComponentFactory.build_all()`, 手动 `/memora rebuild-index` 命令
...
**Q: 索引什么时候需要重建？**
A: 系统启动时 `DatabaseSetup.auto_rebuild_index_if_needed()` 会自动检查。也可手动执行 `/memora rebuild-index`。
