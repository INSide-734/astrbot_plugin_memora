...
当前内置 OneBot 11 适配器；其他协议通过 `IdentityProtocolAdapter` 接口与固定注册表扩展，不在事件处理器或记忆处理器中增加协议分支。
...
- 群名片与历史群显示名只在原群作用域同步；OneBot 私聊昵称只跨 OneBot 私聊同步。assistant 消息、其他协议和其他群不得被改写。
...
- 只有 trusted user 消息且 sender/canonical/label 一致时，才能生成 `stable-identity-v1` 参与者 metadata。参与者按首次出现排序，批次内名称快照取最新值，最多 32 人。
...
- 证据顺序固定为可信稳定来源、原群精确别名、同群成员唯一全局别名、同一 OneBot 私聊当前用户。匹配执行 NFKC 后精确相等；同名多候选或跨 session legacy memory 必须拒绝。
