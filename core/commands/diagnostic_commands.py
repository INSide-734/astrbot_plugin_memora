...
class DiagnosticCommandMixin:
    ...
    async def handle_health(
        ...
        if not callable(provider):
            yield event.plain_result(
                self._diagnostic_component_not_ready("/lmem health")
            )
            return
    ...
    async def handle_diagnostics(
        ...
        """处理 `/lmem diagnostics` 命令。"""
        provider = self._diagnostics_metrics_provider
        if not callable(provider):
            yield event.plain_result(
                self._diagnostic_component_not_ready("/lmem diagnostics")
            )
            return
    ...
    async def handle_trace(
        ...
        if not callable(provider):
            yield event.plain_result(
                self._diagnostic_component_not_ready("/lmem trace")
            )
            return
