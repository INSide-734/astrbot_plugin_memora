...
`core/commands/` 是 Memora 插件命令处理器的 Mixin 实现层，提供 `/memora` 命令组的具体处理方法。通过 Mixin 模式拆分维护类命令和查询类命令，由 `core/command_handler.py` 中的 `CommandHandler` 类组合使用。共 3 个文件。

## 入口与启动

- **组合入口**: `core/command_handler.py` -- `CommandHandler(QueryCommandMixin, MaintenanceCommandMixin)` 继承两个 Mixin，注册到 `core/command_endpoints.py` 的 `/memora` 命令路由
...
所有命令通过 AstrBot 命令系统注册，前缀 `/memora`：
