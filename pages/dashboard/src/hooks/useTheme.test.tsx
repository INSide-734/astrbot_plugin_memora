...
describe("useTheme", () => {
  ...
  it("initializes from bridge context and syncs the DOM theme", () => {
    ...
    expect(localStorage.getItem("memora_theme_override")).toBeNull();
  });

  it("gives a persisted manual override priority over the bridge theme", () => {
    localStorage.setItem("memora_theme", "dark");
    localStorage.setItem("memora_theme_override", "1");
  ...
  it("keeps theme transitions active until the root background tokens finish", () => {
    ...
    expect(document.documentElement.getAttribute("data-theme")).toBe("dark");
    expect(localStorage.getItem("memora_theme")).toBe("dark");
    expect(localStorage.getItem("memora_theme_override")).toBe("1");
