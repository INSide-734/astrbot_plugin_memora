...
describe("createMockConfigServer", () => {
  it("serves the complete authoritative schema with providers and isolated copies", () => {
    ...
    expect(Object.keys(first.schema)).toHaveLength(43);
    expect(countSchemaLeaves(first.schema)).toBe(259);
    ...
    expect(first.schema.reflection_engine).toMatchObject({
      type: "object",
      items: {
        max_parallel_summary_tasks: {
          type: "int",
          default: 4,
          min: 1,
          max: 16,
        },
        max_parallel_summary_tasks_per_session: {
          type: "int",
          default: 2,
          min: 1,
          max: 8,
