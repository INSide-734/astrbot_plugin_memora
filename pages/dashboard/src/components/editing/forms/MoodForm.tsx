...
export function MoodForm({ value, onChange, fieldErrors, disabled = false }: MoodFormProps) {
  ...
  return <EditFormLayout summaryLabel={t("edit.validationSummary")} fieldErrors={{ ...fieldErrors, ...rangeErrors }} focusInvalid={Object.keys(fieldErrors).length > 0 || Object.keys(rangeErrors).length > 0}>
    {({ getFieldError }) => <FieldGroup>
      <Field data-disabled={disabled}><FieldLabel htmlFor="mood-group-id">Group ID</FieldLabel><Input id="mood-group-id" value={value.group_id} disabled={disabled} onChange={(e) => update("group_id", e.currentTarget.value)} /></Field>
      <Field data-invalid={Boolean(error("mood_type"))}><FieldLabel htmlFor="mood-type">Mood type</FieldLabel><Select value={value.mood_type} onValueChange={(next) => next && update("mood_type", next)} disabled={disabled}><SelectTrigger id="mood-type" aria-label="Mood type" aria-invalid={Boolean(error("mood_type"))}><SelectValue /></SelectTrigger><SelectContent><SelectGroup>{MOOD_TYPES.map((item) => <SelectItem key={item.type} value={item.type.toLowerCase()}>{item.label_en}</SelectItem>)}</SelectGroup></SelectContent></Select></Field>
      <Field data-invalid={Boolean(error("intensity"))}><FieldLabel htmlFor="mood-intensity">Intensity</FieldLabel><Input id="mood-intensity" type="number" min="0.1" max="1" step="0.1" value={value.intensity} aria-invalid={Boolean(error("intensity"))} onChange={(e) => bounded("intensity", e.currentTarget.value, 0.1, 1)} />{error("intensity") ? <FieldError id="mood-intensity-error">{error("intensity")}</FieldError> : null}</Field>
      <Field data-invalid={Boolean(error("duration_hours"))}><FieldLabel htmlFor="mood-duration">Duration (hours)</FieldLabel><Input id="mood-duration" type="number" min="0.25" max="168" step="0.25" value={value.duration_hours} aria-invalid={Boolean(error("duration_hours"))} onChange={(e) => bounded("duration_hours", e.currentTarget.value, 0.25, 168)} />{error("duration_hours") ? <FieldError id="mood-duration-error">{error("duration_hours")}</FieldError> : null}</Field>
      <Field><FieldLabel htmlFor="mood-description">Description</FieldLabel><Textarea id="mood-description" value={value.description} disabled={disabled} onChange={(e) => update("description", e.currentTarget.value)} /></Field>
      <div aria-label="Mood history"><FieldLabel>History</FieldLabel><FieldDescription>{value.start_time ?? "—"} {value.is_active === undefined ? "" : String(value.is_active)}</FieldDescription></div>
    </FieldGroup>}
  </EditFormLayout>;
}
