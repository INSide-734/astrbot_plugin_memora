...
export function AffectionForm({ value, onChange, fieldErrors, disabled = false, mode }: AffectionFormProps) {
  ...
  return <EditFormLayout summaryLabel={t("edit.validationSummary")} fieldErrors={{ ...fieldErrors, ...(rangeError ? { affection_score: rangeError } : {}) }} focusInvalid={Boolean(scoreError)}>
    {({ getFieldError }) => <FieldGroup>
      <Field data-disabled={identityDisabled}><FieldLabel htmlFor="affection-user-id">User ID</FieldLabel><Input id="affection-user-id" value={value.user_id} disabled={identityDisabled} onChange={(e) => update("user_id", e.currentTarget.value)} /></Field>
      <Field data-disabled={identityDisabled}><FieldLabel htmlFor="affection-group-id">Group ID</FieldLabel><Input id="affection-group-id" value={value.group_id} disabled={identityDisabled} onChange={(e) => update("group_id", e.currentTarget.value)} /></Field>
      <Field data-invalid={Boolean(scoreError)}><FieldLabel htmlFor="affection-score">Affection score</FieldLabel><Input id="affection-score" type="number" min="-100" max="100" step="1" value={value.affection_score} aria-invalid={Boolean(scoreError)} aria-describedby={scoreError ? "affection-score-error" : undefined} onChange={(e) => { const next = Number(e.currentTarget.value); if (Number.isInteger(next) && next >= -100 && next <= 100) { setRangeError(undefined); update("affection_score", next); } else setRangeError(Number.isInteger(next) ? "Must be between -100 and 100" : "Must be an integer"); }} />{scoreError ? <FieldError id="affection-score-error">{scoreError}</FieldError> : null}</Field>
      <div aria-label="Affection details"><FieldLabel>Level</FieldLabel><FieldDescription>{value.affection_level ?? "—"} {value.level_name ?? "—"}</FieldDescription><FieldLabel>Interaction count</FieldLabel><FieldDescription>{value.interaction_count ?? "—"}</FieldDescription><FieldLabel>Last interaction</FieldLabel><FieldDescription>{value.last_interaction ?? "—"}</FieldDescription></div>
    </FieldGroup>}
  </EditFormLayout>;
}
