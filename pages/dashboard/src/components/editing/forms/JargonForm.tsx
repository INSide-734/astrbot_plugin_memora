...
export function JargonForm({ value, onChange, fieldErrors, disabled = false, mode }: JargonFormProps) {
  ...
  return <EditFormLayout summaryLabel={t("edit.validationSummary")} fieldErrors={validationErrors} focusInvalid={Object.keys(validationErrors).length > 0}>
    {({ getFieldError }) => {
      ...
      return <FieldGroup>
      <Field data-invalid={Boolean(termError)} data-disabled={identityDisabled}><FieldLabel htmlFor="jargon-term">{label("jargon.term")}</FieldLabel><Input id="jargon-term" value={value.term} aria-invalid={Boolean(termError)} aria-describedby={termError?.id} disabled={identityDisabled} onChange={(e) => update("term", e.currentTarget.value)} />{termError ? <FieldError id={termError.id}>{termError.message}</FieldError> : null}</Field>
      <Field data-invalid={Boolean(groupIdError)} data-disabled={identityDisabled}><FieldLabel htmlFor="jargon-group-id">{label("jargon.groupId")}</FieldLabel><Input id="jargon-group-id" value={value.group_id} aria-invalid={Boolean(groupIdError)} aria-describedby={groupIdError?.id} disabled={identityDisabled} onChange={(e) => update("group_id", e.currentTarget.value)} />{groupIdError ? <FieldError id={groupIdError.id}>{groupIdError.message}</FieldError> : null}</Field>
      <Field data-invalid={Boolean(meaningFieldError)}><FieldLabel htmlFor="jargon-meaning">{label("jargon.meaning")}</FieldLabel><Textarea id="jargon-meaning" value={value.meaning} aria-invalid={Boolean(meaningFieldError)} aria-describedby={meaningFieldError?.id} onChange={(e) => { const next = e.currentTarget.value; if (next.trim()) { setMeaningError(undefined); update("meaning", next); } else setMeaningError(label("jargon.meaningRequired")); }} />{meaningFieldError ? <FieldError id={meaningFieldError.id}>{meaningFieldError.message}</FieldError> : null}</Field>
      <Field data-invalid={Boolean(confidenceError)}><FieldLabel htmlFor="jargon-confidence">{label("table.confidence")}</FieldLabel><Input id="jargon-confidence" type="number" min="0" max="1" step="0.01" value={value.confidence} aria-invalid={Boolean(confidenceError)} aria-describedby={confidenceError?.id} onChange={(e) => { const next = Number(e.currentTarget.value); if (Number.isFinite(next) && next >= 0 && next <= 1) { setRangeError(undefined); update("confidence", next); } else setRangeError(label("jargon.confidenceRange")); }} />{confidenceError ? <FieldError id={confidenceError.id}>{confidenceError.message}</FieldError> : null}</Field>
      {(["is_jargon", "is_confirmed", "is_global"] as const).map((field) => { const fieldError = getFieldError(field); return <Field key={field} orientation="horizontal" data-invalid={Boolean(fieldError)} data-disabled={disabled}><Switch aria-label={label(`jargon.${field === "is_jargon" ? "isJargon" : field === "is_confirmed" ? "isConfirmed" : "isGlobal"}`)} aria-invalid={Boolean(fieldError)} aria-describedby={fieldError?.id} checked={value[field]} disabled={disabled} onCheckedChange={(checked) => update(field, checked)} /><div><FieldLabel>{label(`jargon.${field === "is_jargon" ? "isJargon" : field === "is_confirmed" ? "isConfirmed" : "isGlobal"}`)}</FieldLabel><FieldDescription>{label(`jargon.${field === "is_jargon" ? "isJargonDescription" : field === "is_confirmed" ? "isConfirmedDescription" : "isGlobalDescription"}`)}</FieldDescription>{fieldError ? <FieldError id={fieldError.id}>{fieldError.message}</FieldError> : null}</div></Field>; })}
