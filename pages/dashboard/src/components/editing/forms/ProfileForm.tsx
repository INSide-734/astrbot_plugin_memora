  ...
  return <EditFormLayout summaryLabel={t("edit.validationSummary")} fieldErrors={validationErrors} formErrors={formErrors} focusInvalid={Object.keys(validationErrors).length > 0 || formErrors.length > 0}>
    {({ getFieldError, registerField }) => {
      ...
      return <>
        ...
        <Field data-invalid={tagErrors.length > 0} data-disabled={disabled}>
          <FieldLabel>{t("field.tags")}</FieldLabel>
          <div className="flex flex-col gap-3">
            {value.tags.map((tag, index) => {
              const errors = tagFieldErrors[index];
              const tagNumber = index + 1;
              const categoryLabel = index === 0 ? label("profile.tagCategory", "Tag category") : `${label("profile.tagCategory", "Tag category")} ${tagNumber}`;
              const valueLabel = index === 0 ? label("profile.tagValue", "Tag value") : `${label("profile.tagValue", "Tag value")} ${tagNumber}`;
              const confidenceLabel = index === 0 ? label("profile.tagConfidence", "Tag confidence") : `${label("profile.tagConfidence", "Tag confidence")} ${tagNumber}`;
              return (
                <fieldset key={`${tag.category}-${tag.value}-${index}`} className="grid min-w-0 gap-3 rounded-lg border border-border/70 bg-muted/20 p-3 sm:grid-cols-[minmax(8rem,0.9fr)_minmax(0,1.8fr)_minmax(8rem,0.8fr)]">
                  <legend className="col-span-full px-1 text-xs font-medium text-muted-foreground">{label("profile.tagItem", "Tag {0}", String(tagNumber))}</legend>
                  <label htmlFor={`profile-tag-${index}-category`} className="flex min-w-0 flex-col gap-1.5 text-xs font-medium text-muted-foreground">
                    {label("profile.tagCategory", "Tag category")}
                    <Input id={`profile-tag-${index}-category`} ref={(element) => { if (index === 0) registerField("tags", element); registerField(`tags.${index}`, element); registerField(`tags.${index}.category`, element); }} aria-label={categoryLabel} aria-invalid={errors.category.length > 0} aria-describedby={describedBy(errors.category)} disabled={disabled} value={tag.category} onChange={(event) => updateTag(index, "category", event.currentTarget.value)} />
                  </label>
                  <label htmlFor={`profile-tag-${index}-value`} className="flex min-w-0 flex-col gap-1.5 text-xs font-medium text-muted-foreground">
                    {label("profile.tagValue", "Tag value")}
                    <Input id={`profile-tag-${index}-value`} ref={(element) => registerField(`tags.${index}.value`, element)} aria-label={valueLabel} aria-invalid={errors.value.length > 0} aria-describedby={describedBy(errors.value)} disabled={disabled} value={tag.value} placeholder={label("profile.tagValuePlaceholder", "Enter a tag value")} onChange={(event) => updateTag(index, "value", event.currentTarget.value)} />
                  </label>
                  <label htmlFor={`profile-tag-${index}-confidence`} className="flex min-w-0 flex-col gap-1.5 text-xs font-medium text-muted-foreground">
                    {label("profile.tagConfidence", "Tag confidence")}
                    <Input id={`profile-tag-${index}-confidence`} ref={(element) => registerField(`tags.${index}.confidence`, element)} aria-label={confidenceLabel} type="number" min="0" max="1" step="0.01" aria-invalid={errors.confidence.length > 0} aria-describedby={describedBy(errors.confidence)} disabled={disabled} value={tag.confidence} onChange={(event) => updateBoundedNumber(`tags.${index}.confidence`, (next) => updateTag(index, "confidence", next), event.currentTarget.value, 0, 1, label("profile.tagConfidenceRange", "Must be between 0 and 1"))} />
                  </label>
                </fieldset>
              );
            })}
          </div>
          <Button type="button" variant="outline" size="sm" disabled={disabled} onClick={() => update("tags", [...value.tags, { category: "interest", value: "", confidence: 0.5 }])}>{label("profile.addTag", "Add tag")}</Button>
          {tagErrors.map((error) => <FieldError key={error.id} id={error.id}>{error.message}</FieldError>)}
        </Field>
      </>;
    }}
  </EditFormLayout>;
