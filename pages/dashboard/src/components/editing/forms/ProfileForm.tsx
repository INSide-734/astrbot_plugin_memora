...
export function ProfileForm({ value, onChange, fieldErrors, disabled = false, mode }: ProfileFormProps) {
  ...
  return <EditFormLayout summaryLabel={t("edit.validationSummary")} fieldErrors={validationErrors} focusInvalid={Object.keys(validationErrors).length > 0}>
    {({ getFieldError }) => {
      ...
      const preferredTopicErrors = errorsFor(namesUnder("preferences.preferred_topics"));
      const avoidedTopicErrors = errorsFor(namesUnder("preferences.avoided_topics"));
      ...
      return <>
        ...
        <Field data-invalid={preferredTopicErrors.length > 0} data-disabled={disabled}><FieldLabel>{label("profile.preferredTopics", "Preferred topics")}</FieldLabel><TagEditor label={label("profile.preferredTopics", "Preferred topics")} getRemoveLabel={(tag) => label("tags.remove", `Remove ${tag}`, tag)} values={preferences.preferred_topics} onChange={(next) => updatePreferences("preferred_topics", next)} disabled={disabled} ariaInvalid={preferredTopicErrors.length > 0} ariaDescribedBy={describedBy(preferredTopicErrors)} />{preferredTopicErrors.map((error) => <FieldError key={error.id} id={error.id}>{error.message}</FieldError>)}</Field>
        <Field data-invalid={avoidedTopicErrors.length > 0} data-disabled={disabled}><FieldLabel>{label("profile.avoidedTopics", "Avoided topics")}</FieldLabel><TagEditor label={label("profile.avoidedTopics", "Avoided topics")} getRemoveLabel={(tag) => label("tags.remove", `Remove ${tag}`, tag)} values={preferences.avoided_topics} onChange={(next) => updatePreferences("avoided_topics", next)} disabled={disabled} ariaInvalid={avoidedTopicErrors.length > 0} ariaDescribedBy={describedBy(avoidedTopicErrors)} />{avoidedTopicErrors.map((error) => <FieldError key={error.id} id={error.id}>{error.message}</FieldError>)}</Field>
        <Field data-invalid={activeHourErrors.length > 0} data-disabled={disabled}><FieldLabel htmlFor="profile-active-start">{label("profile.activeHours", "Active hours")}</FieldLabel><div className="flex gap-2"><Input id="profile-active-start" aria-label={label("profile.activeHoursStart", "Active hours start")} type="number" min="0" max="23" aria-invalid={activeStartErrors.length > 0} aria-describedby={describedBy(activeStartErrors)} disabled={disabled} value={preferences.active_hours[0] ?? ""} onChange={(event) => updateBoundedNumber("preferences.active_hours.0", (next) => updatePreferences("active_hours", [next, preferences.active_hours[1] ?? next]), event.currentTarget.value, 0, 23, label("profile.activeHoursRange", "Must be between 0 and 23"))} /><Input aria-label={label("profile.activeHoursEnd", "Active hours end")} type="number" min="0" max="23" aria-invalid={activeEndErrors.length > 0} aria-describedby={describedBy(activeEndErrors)} disabled={disabled} value={preferences.active_hours[1] ?? ""} onChange={(event) => updateBoundedNumber("preferences.active_hours.1", (next) => updatePreferences("active_hours", [preferences.active_hours[0] ?? next, next]), event.currentTarget.value, 0, 23, label("profile.activeHoursRange", "Must be between 0 and 23"))} /></div>{activeHourErrors.map((error) => <FieldError key={error.id} id={error.id}>{error.message}</FieldError>)}</Field>
        <Field data-invalid={tagErrors.length > 0} data-disabled={disabled}><FieldLabel>{t("field.tags")}</FieldLabel><div className="flex flex-col gap-3">{value.tags.map((tag, index) => { const errors = tagFieldErrors[index]; return <div key={`${tag.category}-${tag.value}-${index}`} className="flex flex-wrap gap-2"><Input aria-label={index === 0 ? label("profile.tagCategory", "Tag category") : `${label("profile.tagCategory", "Tag category")} ${index + 1}`} aria-invalid={errors.category.length > 0} aria-describedby={describedBy(errors.category)} disabled={disabled} value={tag.category} onChange={(event) => updateTag(index, "category", event.currentTarget.value)} /><Input aria-label={index === 0 ? label("profile.tagValue", "Tag value") : `${label("profile.tagValue", "Tag value")} ${index + 1}`} aria-invalid={errors.value.length > 0} aria-describedby={describedBy(errors.value)} disabled={disabled} value={tag.value} onChange={(event) => updateTag(index, "value", event.currentTarget.value)} /><Input aria-label={index === 0 ? label("profile.tagConfidence", "Tag confidence") : `${label("profile.tagConfidence", "Tag confidence")} ${index + 1}`} type="number" min="0" max="1" step="0.01" aria-invalid={errors.confidence.length > 0} aria-describedby={describedBy(errors.confidence)} disabled={disabled} value={tag.confidence} onChange={(event) => updateBoundedNumber(`tags.${index}.confidence`, (next) => updateTag(index, "confidence", next), event.currentTarget.value, 0, 1, label("profile.tagConfidenceRange", "Must be between 0 and 1"))} /></div>; })}</div><Button type="button" variant="outline" size="sm" disabled={disabled} onClick={() => update("tags", [...value.tags, { category: "interest", value: "", confidence: 0.5 }])}>{label("profile.addTag", "Add tag")}</Button>{tagErrors.map((error) => <FieldError key={error.id} id={error.id}>{error.message}</FieldError>)}</Field>
      </>;
    }}
  </EditFormLayout>;
}
