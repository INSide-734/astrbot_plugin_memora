...
export function SocialRelationForm({ value, onChange, fieldErrors, disabled = false, mode }: SocialRelationFormProps) {
  ...
  return <EditFormLayout summaryLabel={t("edit.validationSummary")} fieldErrors={validationErrors} focusInvalid={Object.keys(validationErrors).length > 0}>
    {({ getFieldError }) => {
      ...
      return <>
      <Field data-invalid={Boolean(getFieldError("from_user"))} data-disabled={identityDisabled}><FieldLabel htmlFor="social-from-user">{label("social.fromUser", "From user")}</FieldLabel><Input id="social-from-user" disabled={identityDisabled} value={value.from_user} onChange={(event) => update("from_user", event.currentTarget.value)} /></Field>
      <Field data-invalid={Boolean(getFieldError("to_user"))} data-disabled={identityDisabled}><FieldLabel htmlFor="social-to-user">{label("social.toUser", "To user")}</FieldLabel><Input id="social-to-user" disabled={identityDisabled} value={value.to_user} onChange={(event) => update("to_user", event.currentTarget.value)} /></Field>
      <Field data-invalid={Boolean(getFieldError("group_id"))} data-disabled={identityDisabled}><FieldLabel htmlFor="social-group-id">{label("social.groupId", "Group ID")}</FieldLabel><Input id="social-group-id" disabled={identityDisabled} value={value.group_id} onChange={(event) => update("group_id", event.currentTarget.value)} /></Field>
      ...
      <Field data-invalid={Boolean(getFieldError("tags"))} data-disabled={disabled}><FieldLabel>{label("field.tags", "Tags")}</FieldLabel><TagEditor label={label("field.tags", "Tags")} getRemoveLabel={(tag) => t("tags.remove", tag)} values={value.tags} onChange={(next) => update("tags", next)} disabled={disabled} />{getFieldError("tags") ? <FieldError>{getFieldError("tags")?.message}</FieldError> : null}</Field>
    </>;
    }}
  </EditFormLayout>;
}
