...
import { DetailField, DetailGrid, DetailSection, DetailTags, DetailText } from "@/components/editing/EntityDetail";
...
export function KnowledgePage({ showToast, navigationTarget, onDirtyChange }: KnowledgePageProps) {
  ...
  return (
    <PageFrame variant="dense" aria-label={t("nav.knowledge")}>
      ...
      <EntityEditorSheet open={detail !== null} onOpenChange={(open) => { if (!open) { if (editDirty) setPendingClose("edit"); else setDetail(null); } }} title={detail?.title ?? ""} description={detail ? (CAT_LABELS[detail.category ?? "fact"] ?? detail.category ?? "") : ""} mode={editMode ? "edit" : "view"} isDirty={editDirty} isSubmitting={editSubmitting} canSave onBeginEdit={() => { setEditFieldErrors({}); setEditFormError(null); setEditMode(true); }} onCancel={() => { if (detail) setEditDraft({ title: detail.title ?? "", content: detail.content ?? "", category: detail.category ?? "fact", confidence: Number(detail.confidence ?? 0), tags: detail.tags ?? [] }); setEditFieldErrors({}); setEditFormError(null); setEditMode(false); setEditDirty(false); }} onSave={saveEdit} labels={{ edit: t("detail.edit"), close: t("common.close"), cancel: t("common.cancel"), save: t("common.save"), saving: t("common.saving") }} status={editDirty ? t("detail.unsaved") : null} view={detail ? <div className="space-y-6"><DetailSection><DetailText>{detail.content}</DetailText></DetailSection><DetailSection><DetailGrid><DetailField label={t("table.category")}>{CAT_LABELS[detail.category ?? "fact"] ?? detail.category}</DetailField><DetailField label={t("table.confidence")}>{formatDashboardNumber(detail.confidence ?? 0, locale, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</DetailField><DetailField label={t("detail.updated")}>{formatDashboardDate(detail.updated_at ?? detail.created_at, locale)}</DetailField><DetailField label={t("kb.accessCount")}>{detail.access_count ?? 0}</DetailField></DetailGrid></DetailSection><DetailSection title={t("field.tags")}><DetailTags tags={detail.tags ?? []} /></DetailSection></div> : null} viewActions={detail ? <Button variant="destructive" size="sm" onClick={() => { setDeleteTarget(detail); setDeleteOpen(true); }}><Trash2 data-icon="inline-start" />{t("common.delete")}</Button> : null} form={<KnowledgeForm value={editDraft} onChange={updateEditDraft} fieldErrors={editFieldErrors} formErrors={editFormError ? [editFormError] : []} disabled={editSubmitting} mode="edit" />} />
