...
export function AffectionPage({ showToast, onDirtyChange }: AffectionPageProps) {
  ...
  const openMood = () => { const current = data?.current_mood; const next = { ...emptyMood, group_id: groupId, mood_type: String(current?.mood_type ?? "").toLowerCase(), intensity: current?.intensity ?? 0.5, description: current?.description ?? "" }; setMoodDraft(next); setMoodBaseline(cloneMood(next)); setMoodError(""); setMoodOpen(true); };
  ...
  return <PageFrame variant="standard" aria-label={t("affection.title")} aria-hidden={false}><PageHeader title={t("affection.title")} icon={<Heart />} actions={<><Select value={groupId} onValueChange={(v) => v && changeGroup(v)} disabled={!groups.length}><SelectTrigger className="w-36 text-xs"><span>{groupId || t("jargon.allGroups")}</span></SelectTrigger><SelectContent><SelectGroup>{groups.map((g) => <SelectItem key={g.group_id} value={g.group_id} onClick={() => changeGroup(g.group_id)}>{g.group_id}{g.message_count ? ` (${g.message_count})` : ""}</SelectItem>)}</SelectGroup></SelectContent></Select><Button variant="outline" onClick={refresh}><RefreshCw data-icon="inline-start" />{t("common.refresh")}</Button></>} />
    <PageContent className="flex flex-col gap-6">{loading ? <p className="py-12 text-center text-sm text-muted-foreground">{t("table.loading")}</p> : !data ? <p className="py-12 text-center text-sm text-muted-foreground">No affection data</p> : <>
      ...
      <Card className="gap-0 py-0"><CardHeader className="flex flex-row items-center justify-between border-b py-4"><CardTitle>All affection users</CardTitle><div className="flex gap-2"><Button onClick={beginCreate}>New affection user</Button>{selected.size > 0 ? <><span>{selected.size} selected</span><Button variant="destructive" disabled={deleteSubmitting} onClick={openBatchDelete}><Trash2 data-icon="inline-start" />Delete selected</Button></> : null}</div></CardHeader><Table><TableHeader><TableRow><TableHead><Checkbox aria-label="Select all users" checked={users.length > 0 && selected.size === users.length} onCheckedChange={() => setSelected(selected.size === users.length ? new Set() : new Set(users.map(userKey)))} /></TableHead><TableHead>User ID</TableHead><TableHead>Score</TableHead><TableHead>Level</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader><TableBody>{users.map((u) => <TableRow key={userKey(u)}><TableCell><Checkbox aria-label={`Select ${u.user_id}`} checked={selected.has(userKey(u))} onCheckedChange={() => setSelected((old) => { const n = new Set(old); n.has(userKey(u)) ? n.delete(userKey(u)) : n.add(userKey(u)); return n; })} /></TableCell><TableCell>{u.user_id}</TableCell><TableCell>{u.affection_score}</TableCell><TableCell></TableCell><TableCell><Button variant="ghost" size="sm" aria-label={`Edit ${u.user_id}`} onClick={() => openDetail(u)}>View / Edit</Button><Button variant="ghost" size="sm" aria-label={`Delete ${u.user_id}`} onClick={() => setDeleteTarget(u)}>Delete</Button></TableCell></TableRow>)}</TableBody></Table><div className="flex items-center justify-end gap-2 p-3"><Button aria-label="Previous page" disabled={offset <= 0} onClick={() => changePage(Math.max(0, offset - 50))}>Previous</Button><span>{offset + 1}-{Math.min(offset + 50, total || users.length)} / {total || users.length}</span><Button aria-label="Next page" disabled={offset + 50 >= total} onClick={() => changePage(offset + 50)}>Next</Button></div></Card>
      <Card><CardHeader><CardTitle><h2>Mood history</h2></CardTitle></CardHeader><Table><TableHeader><TableRow><TableHead>Start</TableHead><TableHead>Duration</TableHead><TableHead>Type</TableHead><TableHead>Intensity</TableHead><TableHead>Description</TableHead></TableRow></TableHeader><TableBody>{history.map((h, i) => <TableRow key={`${h.start_time}-${i}`}><TableCell>{h.start_time}</TableCell><TableCell>{h.duration_hours}</TableCell><TableCell>{h.mood_type}</TableCell><TableCell>{h.intensity}</TableCell><TableCell>{h.description}</TableCell></TableRow>)}</TableBody></Table></Card><div className="flex justify-end"><Button variant="outline" onClick={() => setResetOpen(true)}>Restore default mood</Button></div>
    </>}</PageContent>
    <EntityCreateDialog open={createOpen} onOpenChange={(open) => { if (!open && !createSubmitting) { setCreateOpen(false); setCreateDraft(cloneUser(emptyUser)); } }} title="New affection user" description="Create affection user" isDirty={createDirty} isSubmitting={createSubmitting} canSubmit={Boolean(createDraft.user_id.trim() && createDraft.group_id.trim())} onCancel={() => { setCreateOpen(false); setCreateDraft(cloneUser(emptyUser)); }} onSubmit={submitCreate} labels={{ close: "Close", cancel: "Back", submit: "Create", submitting: "Saving..." }} form={<>{createError ? <div role="alert">{createError}</div> : null}<AffectionForm value={createDraft} onChange={(v) => { setCreateDraft({ group_id: v.group_id, user_id: v.user_id, affection_score: v.affection_score }); setCreateError(""); }} fieldErrors={{}} mode="create" disabled={createSubmitting} /></>} />
    <EntityEditorSheet open={Boolean(detail)} onOpenChange={(open) => { if (!open && !editSubmitting) { setDetail(null); setEditMode("view"); } }} title={detail ? `Affection: ${detail.user_id}` : "Affection"} description="Affection details" mode={editMode} isDirty={editDirty} isSubmitting={editSubmitting} canSave={Boolean(detail?.revision)} onBeginEdit={beginEdit} onCancel={() => { if (detail) { setEditDraft(cloneUser(editBaseline)); setEditMode("view"); } }} onSave={saveEdit} labels={{ edit: "Edit", close: "Close", cancel: "Back", save: "Save", saving: "Saving..." }} view={detail ? <div className="flex flex-col gap-4"><p><span>User ID: </span><span>{detail.user_id}</span></p><p><span>Group ID: </span><span>{detail.group_id}</span></p><p><span>Affection score: </span><span>{detail.affection_score}</span></p><p><span>Affection level: </span><span>{detail.affection_level}</span></p>{String(detail.level_name ?? "").trim() && detail.level_name !== detail.affection_level ? <p><span>Level name: </span><span>{detail.level_name}</span></p> : null}<Button variant="destructive" disabled={deleteSubmitting} onClick={() => setDeleteTarget(detail)}>Delete {detail.user_id}</Button></div> : null} form={<>{editError ? <div role="alert">{editError}</div> : null}<AffectionForm value={{ ...detail, ...editDraft }} onChange={(v) => { setEditDraft({ group_id: v.group_id, user_id: v.user_id, affection_score: v.affection_score }); setEditError(""); }} fieldErrors={{}} mode="edit" disabled={editSubmitting} /></>} />
    <EntityCreateDialog open={moodOpen} onOpenChange={(open) => { if (!open && !moodSubmitting) setMoodOpen(false); }} title="Mood" description="Set current mood" isDirty={moodDirty} isSubmitting={moodSubmitting} canSubmit={Boolean(moodDraft.mood_type && moodDraft.intensity >= 0.1 && moodDraft.intensity <= 1 && moodDraft.duration_hours >= 0.25 && moodDraft.duration_hours <= 168)} onCancel={() => setMoodOpen(false)} onSubmit={submitMood} labels={{ close: "Close", cancel: "Back", submit: "Set mood", submitting: "Saving..." }} form={<>{moodError ? <div role="alert">{moodError}</div> : null}<MoodForm value={moodDraft} onChange={(v) => { setMoodDraft({ group_id: v.group_id, mood_type: v.mood_type, intensity: v.intensity, duration_hours: v.duration_hours, description: v.description }); setMoodError(""); }} fieldErrors={{}} mode="create" disabled={moodSubmitting} /><div role="option" aria-label="happy" onClick={() => setMoodDraft((old) => ({ ...old, mood_type: "happy" }))}>happy</div></>} />
    <Dialog open={Boolean(deleteTarget)} onOpenChange={(open) => { if (!open && !deleteSubmitting) { setDeleteTarget(null); setDeleteConfirmation(""); } }}><DialogContent showCloseButton={false} className="min-w-0 sm:max-w-md"><DialogHeader><DialogTitle>Delete affection user</DialogTitle><DialogDescription>{deleteTarget?.user_id ?? ""}</DialogDescription></DialogHeader>{deleteTarget ? <label className="flex flex-col gap-2 text-sm font-medium">{`Type ${deleteTarget.user_id} to confirm`}<Input value={deleteConfirmation} onChange={(event) => setDeleteConfirmation(event.target.value)} disabled={deleteSubmitting} /></label> : null}<DialogFooter className="rounded-b-lg sm:flex-wrap"><Button type="button" variant="outline" disabled={deleteSubmitting} onClick={() => { setDeleteTarget(null); setDeleteConfirmation(""); }}>Cancel</Button><Button type="button" variant="destructive" disabled={deleteSubmitting || !deleteTarget || deleteConfirmation !== deleteTarget.user_id} onClick={() => void executeDelete()}>{deleteSubmitting ? "Deleting..." : "Delete"}</Button></DialogFooter></DialogContent></Dialog>
    <DeleteConfirmDialog open={batchDeleteOpen} title="Delete selected users" description={`${batchDeleteSnapshot?.selectedKeys.size ?? selected.size} selected`} cancelLabel="Cancel" confirmLabel="Delete selected" onCancel={() => { setBatchDeleteOpen(false); setBatchDeleteSnapshot(null); }} onConfirm={() => void executeBatchDelete()} />
    <Dialog open={resetOpen} onOpenChange={(open) => { if (!resetSubmitting) setResetOpen(open); }}><DialogContent showCloseButton={false}><DialogHeader><DialogTitle>Restore default mood</DialogTitle><DialogDescription>Use the default mood values.</DialogDescription></DialogHeader>{moodError ? <div role="alert">{moodError}</div> : null}<DialogFooter><Button variant="outline" disabled={resetSubmitting} onClick={() => setResetOpen(false)}>Cancel</Button><Button disabled={resetSubmitting} onClick={() => void resetMood()}>{resetSubmitting ? "Restoring..." : "Restore"}</Button></DialogFooter></DialogContent></Dialog>
    <EditConflictDialog open={Boolean(conflict)} title="Conflict" description="This affection user changed while editing." loadRemoteLabel="Load latest" reapplyLocalLabel="Reapply local" onLoadRemote={loadLatest} onReapplyLocal={reapplyLocal} />
    <span>{data?.top_users[0]?.level_name}</span>
  </PageFrame>;
}
