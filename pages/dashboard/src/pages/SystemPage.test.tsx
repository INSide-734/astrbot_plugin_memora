...
describe("SystemPage", () => {
  ...
  it("renders candidate reuse config panel", async () => {
    bridge.apiGet.mockImplementation((endpoint: string) => {
      if (endpoint === "page/stats") {
  ...
  it("disables mode selector when catalog is degraded", async () => {
    bridge.apiGet.mockImplementation((endpoint: string) => {
      ...
      if (endpoint === "page/topic-segmentation/status") {
        return Promise.resolve(
          ok({
            candidate_reuse: {
  ...
  it("validates fixed_k <= max_full_topics constraint", async () => {
    bridge.apiGet.mockImplementation((endpoint: string) => {
      if (endpoint === "page/stats") {
        return Promise.resolve(
          ok({
  ...
  it("shows degraded warning banner when catalog status is not ready", async () => {
    bridge.apiGet.mockImplementation((endpoint: string) => {
      if (endpoint === "page/stats") {
        return Promise.resolve(
          ok({
