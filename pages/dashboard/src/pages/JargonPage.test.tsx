...
describe("JargonPage", () => {
  ...
  it("stored detail opens with context and keeps destructive actions in the footer", async () => { setup(); await showMeanings(); fireEvent.click(screen.getByRole("button", { name: "view term" })); const sheet = await screen.findByRole("dialog", { name: "term" }); expect(await screen.findByText("term context")).toBeTruthy(); expect(screen.queryByRole("textbox", { name: "Meaning" })).toBeNull(); const footer = within(sheet).getByTestId("entity-editor-footer"); const body = within(sheet).getByTestId("entity-editor-body"); expect(within(footer).getByRole("button", { name: /^delete$/i })).toBeTruthy(); expect(within(body).queryByRole("button", { name: /^delete$/i })).toBeNull(); expect(within(sheet).queryByText(/unsaved/i)).toBeNull(); fireEvent.click(within(sheet).getByRole("button", { name: /edit/i })); expect(screen.getByRole("dialog", { name: "term" })).toBe(sheet); fireEvent.change(within(sheet).getByLabelText("Meaning"), { target: { value: "changed" } }); expect(within(sheet).getByText(/unsaved/i)).toBeTruthy(); });
  ...
  it("normalizes prefixed jargon update errors into one linked summary", async () => { setup(); bridge.apiPost.mockRejectedValue(new ApiRequestError("Invalid jargon", "validation_error", { "changes.meaning": "meaning rejected" })); await showMeanings(); fireEvent.click(screen.getByRole("button", { name: "view term" })); const sheet = screen.getByRole("dialog", { name: "term" }); fireEvent.click(within(sheet).getByRole("button", { name: /edit/i })); fireEvent.change(within(sheet).getByLabelText("Meaning"), { target: { value: "kept" } }); fireEvent.click(within(sheet).getByRole("button", { name: /save/i })); await waitFor(() => expect(within(sheet).getAllByRole("alert")).toHaveLength(1)); const href = within(sheet).getByRole("link", { name: "meaning rejected" }).getAttribute("href")!; const errorId = href.slice(1); expect(within(sheet).getByLabelText("Meaning").getAttribute("aria-describedby")?.split(/\s+/)).toContain(errorId); expect(document.querySelectorAll(`[id="${errorId}"]`)).toHaveLength(1); });
  ...
  it("creates a jargon entity, stays in meanings, and opens fresh returned data in view mode", async () => {
    ...
    expect(within(screen.getByRole("dialog", { name: "fresh-term" })).getByText("rev-created")).toBeTruthy();
  });
  ...
  it("returns the entity editor to view mode with the update response revision", async () => {
    ...
    expect(within(await screen.findByRole("dialog", { name: "term" })).getByText("rev-updated")).toBeTruthy();
    expect(screen.queryByLabelText("Meaning")).toBeNull();
  });
  ...
  it("retains the local draft on edit conflict and retries exactly against the latest revision", async () => {
    ...
    expect(within(await screen.findByRole("dialog", { name: "term" })).getByText("rev-retried")).toBeTruthy();
  });
