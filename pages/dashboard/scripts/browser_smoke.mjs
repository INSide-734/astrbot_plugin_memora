...
async function installBundledMockBridge(page) {
  const content = `{
    ...
    const createNoUpdateSmokeBridge = ${createNoUpdateSmokeBridge.toString()};
    window.__memoraLoseNextStaleApplyResponse = true;
    installBundledMockBridgeHarness(window, (sourceBridge, options) =>
      createNoUpdateSmokeBridge(sourceBridge, instrumentBrowserBridge, {
        ...options,
...
async function waitForConfigReady(page, label) {
  ...
  // memory_dedup 四叶加入后 255→259；`metrics_retention_days` 叶加入后 259→260；
  // semantic_mode/semantic_threshold 两叶加入后 260→262（节数不变）。
  const expectedSections = 43;
  const expectedFields = 262;
  await waitForRootText(
    page,
