...
describe("browser smoke helpers", () => {
  ...
  it("redacts sensitive bridge-call snapshots without changing forwarded transport values", async () => {
    ...
    assert.equal(await bridge.apiGet("page/injection-strategy/decisions/detail", params),
      getResponse,
    );
    assert.equal(await bridge.apiPost("page/config/apply", body), postResponse);
  ...
  it("returns no update while forwarding other smoke bridge requests", async () => {
    const requests = [];
    const sourceBridge = {
      async apiGet(endpoint, params) {
    ...
    const { bridge, calls } = createNoUpdateSmokeBridge(
      sourceBridge,
      instrumentBrowserBridge,
    );

    assert.deepEqual(await bridge.apiGet("page/update/check", { ignored: "false" }), {
      status: "ok",
      data: { enabled: true, available: false, ignored: false, release: null },
    });
    assert.deepEqual(await bridge.apiGet("page/config/state", { revision: "r1" }), {
      status: "ok",
      data: { endpoint: "page/config/state" },
    });
    assert.deepEqual(requests, [{ endpoint: "page/config/state", params: { revision: "r1" } }]);
    assert.deepEqual(calls[0].response, {
      status: "ok",
      data: { enabled: true, available: false, ignored: false, release: null },
    });
  });
