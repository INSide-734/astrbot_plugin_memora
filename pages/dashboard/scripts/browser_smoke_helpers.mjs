...
/** 为页面 smoke 提供无更新响应，避免全局更新对话框遮挡页面交互。 */
export function createNoUpdateSmokeBridge(sourceBridge, instrument, options = {}) {
  const smokeBridge = Object.create(sourceBridge);
  smokeBridge.apiGet = (endpoint, params) => (
    endpoint === "page/update/check"
      ? Promise.resolve({
        status: "ok",
        data: { enabled: true, available: false, ignored: false, release: null },
      })
      : sourceBridge.apiGet(endpoint, params)
  );
  return instrument(smokeBridge, options);
}
