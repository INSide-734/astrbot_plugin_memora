...
- 记忆列表新增来源完整性、作用域与隐私读取门：`privacy_level` 缺失或为 null 的历史行按 `shared` 回退（与 `canonical_source_validation` 同一口径，请求端的隐私筛选沿用该回退值），键存在但取值越界或非文本的记录仍按非法排除。
- 记忆原子（Atom）降级为派生信号：模型可见正文一律取当前 canonical 事实原文，canonical 更新后按当前事实重派生，父记忆已失效的信号不再进入前瞻注入与图事实派生；对外统计的原子总数只计入父记忆当前有效且可召回的信号（`count_current_atoms`），按类型分布（`count_by_type`）仍为原始口径。
