...
- 记忆正文更新改为两阶段收敛：新内容先落为不可召回的暂存记录，再在单个事务内切换可见性并删除旧记录；任一步失败都由写修复收敛，保证任一时刻最多一条可召回记录（失败原因码 `content_replace_failed`），对同一记录重复发起替换会被安全拒绝。
...
- 跨窗口近重复合并新增事实对齐门：待并入的既有记忆若其 `key_facts` 与自身正文已不对齐，不再被强化合并（避免用错误事实强化旧记录），改为按冲突回落新增一条 canonical，并计入 `dedup_merge_conflict`。
- 记忆列表对历史行的隐私字段采用 canonical 读取回退口径：`privacy_level` 缺失或为 null 的记录按 `shared` 参与可见性判定（此前被直接隐藏），取值越界或非法的记录仍保持隐藏；请求端的隐私筛选沿用同一回退值。
- 记忆原子（Atom）降级为派生信号：模型可见正文一律取当前 canonical 事实原文，canonical 更新后按当前事实重派生，父记忆已失效的信号不再进入前瞻注入与图事实派生；统计口径改为只计入父记忆当前有效且可召回的信号。
...
Memora 1.3.0 引入话题 top-K 候选复用与可恢复的并行总结调度：总结窗口可从当前作用域的近期高频话题目录复用固定数量候选，自动反思、手动总结与旧 pending 恢复统一为带 claim fencing 的固定窗口任务；同时移除会把 AstrBot 框架本体重复装入插件目录的 `astrbot` 运行时依赖声明。
...
- 对齐来源引用契约：Prompt、Pydantic guardrail 和质量门禁统一支持每条记忆最多 16 条 `source_refs`，并在反思窗口内使用同一门禁快照。
- 修复 guardrail 失败回退时只读取顶层字段的问题；嵌套 `memories[]` 输出现会保留候选并提升首条兼容字段，避免正常摘要被默认值覆盖。
- 修复 grounding 将可信身份标签中的稳定标识数字和由消息时间戳确定性推导的当前日期误判为编造数字的问题；未可信身份数字和真实数值冲突仍继续隔离。

### 升级说明

- 从 `1.2.3` 升级无需迁移配置、canonical memory、索引或数据库。
...
- 从 `1.2.1` 升级无需迁移配置、canonical memory、索引或数据库。
