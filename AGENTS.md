...
3. 安全、隐私、数据权威与宿主平台约束优先于其他协作规则。
4. 根级 `AGENTS.md` 提供全局基线；最近目录的 `AGENTS.md` 可增加或收紧要求，但不得放宽本页的安全、质量和验证要求。
5. 对运行时行为，生产代码、Schema/迁移、可执行测试和构建脚本是事实来源。文档与实现冲突时先核对事实，不得用改文档掩盖缺陷。
6. 任务说明只限定本次范围，不能推翻已发布的公开契约。需求、实现或验收标准不明确时先查询代码、调用方和测试；仍有关键歧义时再向人确认。
