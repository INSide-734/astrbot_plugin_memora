...
需要提交问题报告时，可在 Dashboard 配置页开启“调试模式（问题报告）”；保存成功后开关会立即作用于当前插件进程，不依赖 AstrBot 热重载。复现问题后，可以从 AstrBot 日志筛选 `[MemoraDebug]` 行，或提交插件数据目录下的相对路径 `diagnostics/memora-debug.jsonl`；智能控制台的召回追踪也会显示独立的“问题报告 Debug”状态并写入脱敏完成事件。“候选评分明细”只表示本次是否产生候选计分轨迹，零候选时为关闭状态不代表问题报告 Debug 失效。文件按大小轮转，包含当前文件和两个备份文件，每个文件最大 `5 MB`。诊断记录只包含阶段、状态、耗时、计数、路由和安全异常摘要，不记录对话、查询、Prompt、模型回复、记忆正文、用户/群组/会话身份或 Provider 凭据。问题复现并收集日志后，请关闭该开关。
...
| `/memora update [check\|download\|apply]` | 检查、下载或安装经 SHA-256 校验的 runtime 更新包；默认执行 `check`。 |
| `/memora help` | 查看帮助。 |
