...
- 内置 OneBot 11 严格身份适配器：可信用户以 QQ 号作为稳定用户 ID，群名片和昵称只作为当前显示名称，改名不会创建第二个用户。
- 新名称会按群聊或私聊作用域更新身份目录，并同步已保存会话消息的显示名称；旧名称只保留为作用域别名，供唯一证据成立时只读召回。
- 长期记忆参与者使用 `QQ:10001` 这类稳定标签；历史记忆无需重写，召回时只在可信来源、群精确别名或同一 OneBot 私聊用户能唯一确认时临时补充身份说明。
- 身份目录只幂等创建独立表，不执行 `ALTER TABLE`、历史扫描、迁移或回填；目录不可用时保留严格解析并关闭名称持久化增强，不阻断聊天。
- 协议解析由可扩展适配器接口承载；当前内置 OneBot 11，匿名、冲突或非法事件不会绑定到可跨会话关联的 QQ 用户。
