...
def test_readme_documents_current_command_set() -> None:
    ...
    for command in command_names:
        assert f"/lmem {command}" in readme
...
def test_documented_commands_match_command_endpoints() -> None:
    ...
    commands = re.findall(r'@lmem\.command\("([^"]+)"', command_source)
    assert commands, "No /lmem commands found in core/command_endpoints.py"
    ...
    for relative_path in documented_files:
        ...
        for command in commands:
            assert f"/lmem {command}" in text, (
                f"{relative_path} is missing documented command: /lmem {command}"
            )
