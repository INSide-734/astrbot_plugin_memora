            ...
            for index in range(total):
                ...
                for variant in ("strict_full", *(f"top_k_{k}" for k in TOP_K_VALUES)):
                    record = create_replay_record(
                        case_hash=case, scale_bucket=bucket, chat_type=chat,
                        catalog_generation=1, catalog_topic_count=count,
                        variant=variant, source_window_digest=f"digest-{case}",
                        candidate_count=4, bm25_hit_count=4, recent_fill_count=0,
                        identity_drop_count=0, safety_violation_count=0,
                        exact_reuse_count=98, exact_topic_count=100,
                        candidate_occurrence_count=98, misreuse_count=0, duplicate_count=0,
                        window_topic_count=100, fragmentation_count=5,
                        token_availability="estimated",
                        estimated_tokens=1000 if variant == "strict_full" else 700,
                        selector_duration_ms=1.0, provider_duration_ms=100.0,
                        reason_code="measured", is_negative_window=negative,
                    )
...
def _result(report, k=4, chat="private", count=17):
    ...
    return next(result for result in report.bucket_results
                if result.k == k and result.chat_type == chat and result.catalog_topic_count == count)


@pytest.mark.parametrize("payload", [
    {"topic": "secret"}, {"metadata": {"query": "secret"}},
    {"items": [{"Scope": "secret"}]}, {"PROMPT": "secret"},
])
...
def test_exact_count_is_preserved_and_minimum_demonstrated_threshold_is_selected():
    ...
    assert (report.recommended_k,
            report.recommended_activation_threshold) == (4, 17)
    assert report.recommended_bucket_modes["small"] == "top_k"
    assert all(mode == "observe" for bucket,
               mode in report.recommended_bucket_modes.items() if bucket != "small")
    assert {(result.catalog_topic_count, result.chat_type, result.k)
            for result in report.bucket_results} == {
                (17, chat, k) for chat in ("private", "group") for k in TOP_K_VALUES
    }
    assert all(result.decision == "accept" and result.quality_sample_count == 200
               and result.negative_sample_count == 100 for result in report.bucket_results)
...
def test_k24_can_be_the_smallest_passing_k():
    ...
    records = [replace(record, fragmentation_count=8)
               if record.variant not in ("strict_full", "top_k_24") else record
               for record in _records(count=64, bucket="medium")]
    report = _gate(records)
    assert (report.recommended_k,
            report.recommended_activation_threshold) == (24, 64)
    assert all(result.decision == ("accept" if result.k == 24 else "reject")
               for result in report.bucket_results)


def test_threshold_must_exclude_every_failing_tested_lower_count():
    """同桶较小规模不通过时，保留结果并提升到真正通过的实测规模。"""
    records = [replace(record, provider_duration_ms=120.0)
               if record.variant != "strict_full" else record for record in _records()]
    records += _records(count=23)
    report = _gate(records)
    assert (report.recommended_k,
            report.recommended_activation_threshold) == (4, 23)
...
def test_higher_count_failure_prevents_enabling_lower_success():
    ...
    records = _records() + [replace(record, provider_duration_ms=120.0)
                            if record.variant != "strict_full" else record for record in _records(count=23)]
    report = _gate(records)
...
def test_minimum_k_precedes_minimum_threshold():
    ...
    records = [replace(record, fragmentation_count=8)
               if record.variant == "top_k_4" else record for record in _records()]
...
def test_failed_other_bucket_stays_observe_without_hiding_its_results():
    ...
    records = _records() + [replace(record, provider_duration_ms=None)
                            for record in _records(count=64, bucket="medium")]
    report = _gate(records)
    assert report.recommended_bucket_modes["small"] == "top_k"
    assert report.recommended_bucket_modes["medium"] == "observe"
    assert any(result.scale_bucket == "medium" and result.decision == "reject"
               for result in report.bucket_results)
...
def test_missing_chat_has_explicit_rejection_for_every_k():
    ...
    assert all(result.decision ==
               "reject" and "stratum_missing" in result.reason_codes for result in group)
...
def test_legacy_missing_scale_cannot_enable_rollout():
    ...
    assert report.recommended_activation_threshold is None
    assert all(
        "catalog_scale_unavailable" in result.reason_codes for result in report.bucket_results)
...
def test_negative_samples_never_inflate_normal_minimum_even_with_overrides():
    ...
    report = _gate(_records(normal_n=199, negative_n=250), min_quality_n=1,
                   min_token_n=1, min_latency_n=1, min_negative_n=1)
    assert report.recommended_k is None
    result = _result(report)
    assert (result.quality_sample_count, result.token_sample_count,
            result.latency_sample_count) == (199, 199, 199)
    assert result.negative_sample_count == 250


@pytest.mark.parametrize(("changes", "counter", "reason"), [
    ...
    ({"provider_duration_ms": float("nan")},
     "latency_sample_count", "sample_short_latency"),
])
...
@pytest.mark.parametrize("mutation", ["duplicate", "digest", "generation", "negative", "source_alias"])
def test_duplicate_or_mismatched_source_pairs_are_rejected(mutation):
    ...
    index = next(i for i, record in enumerate(
        records) if record.variant == "top_k_4")
    ...
    assert {"pair_duplicate", "pair_source_mismatch"} & set(
        result.reason_codes)


def test_failed_negative_execution_is_not_counted_as_safety_evidence():
    """selector 降级未运行负向选择时，不得当作零违规成功。"""
    records = [replace(record, execution_status="error")
               if record.is_negative_window and record.variant == "top_k_4" else record
...
def test_unavailable_provider_and_token_measurements_stay_unavailable():
    ...
    records = [replace(record, estimated_tokens=None, token_availability="unavailable",
                       provider_duration_ms=None) for record in _records()]
    report = _gate(records)
...
def test_misreuse_uses_occurrence_percentage_points_and_rejects_missing_annotation():
    ...
    records = [replace(record, candidate_occurrence_count=20, misreuse_count=2)
               if record.variant == "top_k_4" else record for record in _records()]
    result = _result(_gate(records))
    assert result.misreuse_rate == 10.0
    assert "misreuse_guardrail" in result.reason_codes
    measured = records[1]
    assert _compute_misreuse_rate(
        [measured, replace(measured, misreuse_count=None)]) is None
...
def test_provider_tail_regression_is_rejected_by_paired_p95():
    ...
    for index, record in enumerate(records):
        if record.variant == "top_k_4" and not record.is_negative_window and changed < 20:
