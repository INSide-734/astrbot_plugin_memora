...
from core.features.reflection.application.topic_candidate_selector import TopicCandidateSelector
...
def _document(identifier, labels, **metadata):
    ...
    return {
        "id": identifier, "text": "合成历史，不来自生产",
        "metadata": {
            "topics": labels, "scope_key": "scope-a", "chat_type": "private",
            "privacy_level": "shared", "resolver_revision": "resolver-1",
            "source_provenance_complete": True, **metadata,
        },
    }
...
async def catalog():
    ...
    documents = [_document(index + 1, [f"话题 {index:03}"])
...
def _windows(**scenario):
    ...
    return create_synthetic_windows([{
        "bucket": "medium", "chat_type": "private", "scope_key": "scope-a",
        "privacy_level": "shared", "resolver_revision": "resolver-1",
        "semantic_clusters": ["话题 000"], "fragmentation_count": 0,
...
def _configs():
    ...
    common = ReplayConfig(
        mode="full", activation_threshold=3, fixed_k=4,
        max_full_topics=24, max_full_prompt_tokens=2000,
        max_query_chars=2000, overfetch_factor=3, metrics_retention_days=30,
    )
    return {
        "off": replace(common, mode="off"), "strict_full": common,
...
async def test_real_replay_pairs_all_k_and_runs_k24(catalog):
    ...
    assert set(by_variant) == {"off", "strict_full",
...
async def test_strict_full_reads_canonical_when_derived_index_is_missing(catalog):
    ...
    await db.commit()
    configs = _configs()
    cases = await replay_window(_windows()[0], selector, {
        "strict_full": configs["strict_full"], "top_k_4": configs["top_k_4"],
    }, catalog_store=store)
...
async def test_strict_full_is_unbounded_but_observe_remains_bounded(catalog):
    ...
    configs = _configs()
    cases = await replay_window(_windows()[0], selector, {
        "strict_full": replace(configs["strict_full"], max_full_topics=4, max_full_prompt_tokens=50),
...
async def test_raw_annotations_cannot_become_provider_measurements(catalog):
    ...
    _, store, selector = catalog
    windows = _windows(provider_durations_ms={
                       "strict_full": 40, "top_k_24": 20})
...
async def test_invalid_k_is_error_not_privacy_failure_or_success(catalog):
    ...
    cases = await replay_window(_windows()[0], selector, {"top_k_24": config}, catalog_store=store)
    case = cases[0]
...
async def test_catalog_generation_is_measured_not_hardcoded(catalog):
    ...
    await db.commit()
    cases = await replay_window(_windows()[0], selector,
                                {"strict_full": _configs()["strict_full"]}, catalog_store=store)
...
async def test_cancellation_is_not_an_error_record(catalog, monkeypatch):
    ...
    with pytest.raises(asyncio.CancelledError):
        await replay_window(_windows()[0], selector,
                            {"top_k_4": _configs()["top_k_4"]}, catalog_store=store)
