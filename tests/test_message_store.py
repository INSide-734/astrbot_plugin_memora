...
class TestMessageStoreRemaining:
    ...
    async def test_trim_with_summarized_index_exceeding_actual(self, tmp_db_path):
        ...
        try:
            ...
            result = await store.trim_if_safe("sess-overflow", 1, 5)
    ...
    async def test_delete_old_sessions_with_ttl_seconds(self, tmp_db_path):
        """TTL 删除应递增 epoch，并回调取消对应本地 worker。"""
        store = ConversationStore(tmp_db_path)
        await store.initialize()
        try:
            ...
            cancel_jobs = AsyncMock(return_value=1)
            deleted = await store.delete_old_sessions(
                ttl_seconds=1800,
                cancel_jobs_cb=cancel_jobs,
            )
            assert deleted >= 1
            assert await store.get_session("ttl-sess") is None
            cancel_jobs.assert_awaited_once_with("ttl-sess", 1)
        finally:
            await store.close()
