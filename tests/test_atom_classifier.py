...
class TestNegationDetection:
    ...
    def test_genuine_preference_still_works(self):
        result = classify_atoms(
            ["很喜欢咖啡"], fact_evidence(["很喜欢咖啡"]), enable_quality_filter=False
        )
        assert len(result) == 1
        assert result[0].atom_type.value == "preference"
    ...
    def test_english_negation_not_preference(self):
        ...
        result = classify_atoms(
            ["I don't like coffee"],
            fact_evidence(["I don't like coffee"]),
            enable_quality_filter=False,
        )
        atom = result[0]
        assert atom.atom_type.value == "preference"
    ...
    def test_never_like_is_not_preference(self):
        result = classify_atoms(
            ["从不吃辣"], fact_evidence(["从不吃辣"]), enable_quality_filter=False
        )
        assert len(result) == 1
        assert result[0].atom_type.value != "preference"
...
class TestAtomClassification:
    ...
    def test_unknown_classification(self):
        ...
        result = classify_atoms(
            ["嗯嗯好的"], fact_evidence(["嗯嗯好的"]), enable_quality_filter=False
        )
        assert result[0].atom_type.value == "unknown"

    def test_confidence_bounds(self):
        result = classify_atoms(
            ["喜欢喝咖啡"], fact_evidence(["喜欢喝咖啡"]), enable_quality_filter=False
        )
        assert 0.0 <= result[0].confidence <= 1.0

    def test_empty_string_handling(self):
        """空字符串不应产生原子."""
        result = classify_atoms([""], fact_evidence([""]), enable_quality_filter=False)
        assert len(result) == 0
    ...
    def test_multiple_atoms_batch(self):
        ...
        results = classify_atoms(
            facts, fact_evidence(facts), enable_quality_filter=False
        )
...
class TestNegationDetectionExtended:
    ...
    def test_zh_negation_not_classified_as(self, content, excluded_type):
        ...
        result = classify_atoms(
            [content], fact_evidence([content]), enable_quality_filter=False
        )
        assert len(result) == 1
    ...
    def test_all_negations_produce_valid_atoms(self):
        ...
        results = classify_atoms(
            all_s, fact_evidence(all_s), enable_quality_filter=False
        )
...
def test_evidence_boundary_survives_disabled_quality_filter():
    facts = ["用户喜欢茶", "用户住在月球"]
    groups = fact_evidence(facts)
    groups[1][0]["role"] = "assistant"
    atoms = classify_atoms(facts, groups, enable_quality_filter=False)
    assert [atom.content for atom in atoms] == [facts[0]]
    assert atoms[0].source_evidence == groups[0]
    assert classify_atoms([facts[0]], [[]], enable_quality_filter=False) == []
    with pytest.raises(ValueError, match="grounding_fact_evidence_mismatch"):
        classify_atoms(facts, [groups[0]], enable_quality_filter=False)
