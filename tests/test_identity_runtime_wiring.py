...
async def test_factory_identity_store_failure_returns_resolver_only_runtime(
    ...
    from core.features.identity.application.runtime import ProtocolIdentityRuntime
    from core.platform.composition.identity_component_factory import (
    ...
    monkeypatch.setattr(
        "core.platform.composition.identity_component_factory.ProtocolIdentityStore",
        MagicMock(return_value=store),
    )

    runtime = await build_identity_runtime(str(tmp_path), MagicMock())
...
async def test_factory_identity_runtime_propagates_initialization_cancellation(
    ...
    with pytest.raises(asyncio.CancelledError):
        await build_identity_runtime(str(tmp_path), MagicMock())
...
async def test_factory_identity_runtime_builds_service_and_closes_owned_store(
    ...
    runtime = await build_identity_runtime(str(tmp_path), manager)
