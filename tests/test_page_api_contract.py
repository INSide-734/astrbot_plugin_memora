...
def test_candidate_reuse_status_response_schema() -> None:
    """真实话题配置 API 必须通过 Store 摘要端口返回低敏观测契约。"""
    import asyncio
    ...
    class MockCatalogStore:
        ...
        async def get_catalog_summary(self):
            """返回实际 handler 消费的固定字段，不提供 SQL 连接。"""
            return {
                "catalog_status": "ready",
                "dirty_count": 2,
                "scope_buckets": {"small": 5, "medium": 3, "large": 1},
            }
    ...
    class Stub:
        ...
        _get_candidate_reuse_status = (
            TopicSegmentationApiMixin._get_candidate_reuse_status
        )
        _query_aggregated_metrics = TopicSegmentationApiMixin._query_aggregated_metrics

        async def _ensure_plugin_ready(self):
            memory_engine = MagicMock()
            memory_engine.topic_catalog_store = MockCatalogStore()
    ...
    stub.plugin.config_manager = MagicMock()
    stub.plugin.config_manager.get.side_effect = {
        "topic_segmentation.enabled": True,
        "topic_segmentation.strategy": "a_b_hybrid",
    }.get

    result = asyncio.run(stub.get_topic_segmentation_config())

    assert result["status"] == "ok"
    assert "candidate_reuse" in result["data"]
