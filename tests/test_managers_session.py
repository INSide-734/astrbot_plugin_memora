...
class TestSessionLifecycleMixin:
    ...
    async def test_clear_session(self) -> None:
        """clear_session 通过 Store 原子 fence 并清理缓存。"""
        mgr = _TestSessionManager()
        mgr.store.get_summary_epoch = AsyncMock(return_value=(1, 0))
        mgr.store.clear_session_atomically = AsyncMock(return_value=2)

        await mgr.clear_session("s1")

        mgr.store.clear_session_atomically.assert_awaited_once_with("s1")

    @pytest.mark.asyncio
    async def test_clear_session_propagates_atomic_reset_failure(self) -> None:
        """原子来源清理失败时不得报告成功。"""
        mgr = _TestSessionManager()
        mgr.store.get_summary_epoch = AsyncMock(return_value=(1, 0))
        mgr.store.clear_session_atomically = AsyncMock(
            side_effect=RuntimeError("atomic reset failed")
        )

        with pytest.raises(RuntimeError, match="atomic reset failed"):
            await mgr.clear_session("s1")
    ...
    async def test_cleanup_expired_sessions(self) -> None:
        ...
        scheduler = MagicMock()
        scheduler.cancel_session_jobs = AsyncMock(return_value=1)
        mgr.summary_scheduler = scheduler
        ...
        assert result == 5
        mgr.store.delete_old_sessions.assert_called_once_with(
            ttl_seconds=7200,
            cancel_jobs_cb=scheduler.cancel_session_jobs,
        )
