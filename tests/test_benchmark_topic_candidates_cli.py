...
def test_full_execution_publishes_every_actual_stratum_and_success_status(tmp_path):
    ...
    assert {(entry["scale_bucket"], entry["catalog_topic_count"])
            for entry in report["bucket_results"]} == {("tiny", 4), ("small", 16), ("medium", 50)}
    assert len(report["bucket_results"]) == 3 * 2 * 5
    assert report["paired_case_count"] == 16
    assert report["variant_record_counts"]["top_k_24"] == 24
    assert report["variant_status_counts"]["top_k_24"] == {
        "success": 16, "degraded": 8}
    ...
    assert all(entry["decision"] ==
               "reject" for entry in report["bucket_results"])
    assert all(entry["latency_sample_count"] == entry["token_sample_count"] == 0
...
async def test_selector_fallbacks_do_not_count_as_successful_pairs(monkeypatch):
    ...
    report = _run_benchmark(records, config, 200, 200,
                            200, paired, variants, statuses)
    assert report["recommended_k"] is None
    assert all(entry["quality_sample_count"] == entry["negative_sample_count"] == 0
               for entry in report["bucket_results"])
    assert all("replay_not_successful" in entry["reason_codes"]
               for entry in report["bucket_results"])
