...
class TestEndpointDelegation:
    ...
    @pytest.mark.asyncio
    async def test_summarize_delegates_optional_action(self) -> None:
        plugin = _Plugin()
        handler = MagicMock()
        ...
        results = await _collect(plugin.summarize(event, "confirm-abandon"))

        assert results == ["queued", "done"]
        handler.handle_summarize.assert_called_once_with(event, "confirm-abandon")
