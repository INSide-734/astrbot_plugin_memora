...
@pytest.mark.parametrize(
    ("gate_action", "expected_marks", "expects_canonical_write"),
    [("allow", 1, True), ("quarantined", 0, False), ("mark_write", 0, True)],
)
async def test_reflection_pipeline_marks_only_canonical_topics(
    gate_action: str,
    expected_marks: int,
    expects_canonical_write: bool,
) -> None:
    """仅 allow canonical 写入可标记连续性，mark_write 必须隔离。"""
    tracker = MagicMock()
    engine = MagicMock()
    ...
    await store_reflection_candidates(
        ...
        schedule_evolution_after_write=schedule_evolution,
    )

    assert tracker.mark_topics.call_count == expected_marks
    if expects_canonical_write:
