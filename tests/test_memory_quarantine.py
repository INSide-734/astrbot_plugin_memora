...
def _approval_conversation(
    message: Message | None = None,
) -> StableApprovalConversation:
    """构造满足来源 epoch、序号和锁契约的批准会话替身。"""
    return StableApprovalConversation(message or _source_message())
...
async def test_approve_creates_one_real_canonical_memory(
    ...
    conversation_manager = StableApprovalConversation(message)
    gate = MemoryQualityGate(
        quarantine_store,
        memory_engine=engine,
        memory_processor=processor,
...
async def test_approval_repair_fails_closed_for_wrong_token_or_canonical(
    ...
    engine.get_memory = AsyncMock(
        return_value={
            "text": "用户喜欢咖啡。",
            "metadata": {
                **source_correlation(source_window),
...
async def test_approve_resolves_needs_judge_via_judge(
    ...
    staged = await quarantine_store.stage_candidate(
        ...
        persona_id="persona-1",
        source_window={
            **stable_source_window(_source_message()),
            "group_id": "group-1",
        },
        is_group_chat=True,
    )
...
async def test_approval_cancellation_before_write_blocks_and_propagates(
    ...
    engine.add_memory = AsyncMock()
    conversation_manager = _approval_conversation()
    conversation_manager.get_messages_seq_range = AsyncMock(
...
async def test_unknown_canonical_write_result_prevents_automatic_retry(
    ...
    conversation_manager = _approval_conversation()
    conversation_manager.get_messages_seq_range = AsyncMock(return_value=[message])
    gate = MemoryQualityGate(
        quarantine_store,
        memory_engine=engine,
        memory_processor=processor,
