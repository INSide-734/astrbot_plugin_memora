...
class TestCreateBackup:
    ...
    async def test_version_backup_quiesces_bound_summary_scheduler(
        ...
        """版本变更异步备份也必须暂停并恢复总结调度器。"""
        order: list[str] = []
        scheduler = SimpleNamespace(
            pause=AsyncMock(side_effect=lambda: order.append("pause")),
            resume=AsyncMock(side_effect=lambda: order.append("resume")),
        )
        ...
        await manager.backup_if_needed_async()
