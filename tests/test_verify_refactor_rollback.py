...
async def test_fixture_is_current_for_production_schema_manager(tmp_path: Path) -> None:
    ...
    fixture 早于 topic catalog 表引入，因此除 catalog 补建外不应有
    任何其他迁移项：列/表/索引/触发器/幂等映射全部无需变更。
    """
    ...
    assert plan.topic_catalog_repair_required is True
    ...
    assert plan.missing_triggers == (
        "topic_catalog_documents_delete",
        "topic_catalog_documents_insert",
        "topic_catalog_documents_update",
    )
    assert plan.idempotency_rebuild_required is False
    assert plan.write_journal_required is False
